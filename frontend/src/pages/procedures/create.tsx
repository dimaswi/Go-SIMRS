...
import { proceduresApi, TARIFF_COMPONENTS, PROCEDURE_TYPES } from "@/lib/api/procedures";
import { masterDataApi, type MasterData } from "@/lib/api/master-data";
import type { CreateProcedureRequest, PatientClass, TariffRequest, ProcedureType } from "@/lib/api/procedures";
...
export default function ProcedureCreate() {
  ...
  const fetchMasterData = async () => {
    try {
      ...
      const data = response.data.data as unknown as MasterDataState;
      setMasterData(data);
      
      // Initialize tariffs based on patient_class master data
      if (data.patient_class && data.patient_class.length > 0) {
        const initialTariffs: TariffRequest[] = data.patient_class.map(pc => ({
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Tambah tindakan medis baru dengan tarif per kelas pasien"
        actions={
