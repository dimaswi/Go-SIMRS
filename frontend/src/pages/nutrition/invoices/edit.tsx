...
export default function NutritionInvoiceEdit() {
  ...
  useEffect(() => {
    setPageTitle("Edit Faktur Bahan Gizi");
    if (id) {
      loadInitialData(Number(id));
    }
  }, [id]);
  ...
  const loadInitialData = async (invoiceID: number) => {
    setLoadingData(true);
    try {
      const [ingredientsRes, invoiceRes] = await Promise.all([
        nutritionIngredientApi.getAll({ limit: 500, is_active: "true" }),
        nutritionIngredientInvoiceApi.getById(invoiceID),
      ]);
      setIngredientsMaster(ingredientsRes.data.data || []);
      ...
      const mappedItems = (invoice.items || []).map((row: any) => ({
        ingredient_id: row.ingredient_id || 0,
        quantity: row.quantity || 0,
        unit: row.unit || "kemasan",
        unit_price: row.unit_price || 0,
        notes: row.notes || "",
      }));
      setItems(mappedItems.length > 0 ? mappedItems : [{ ingredient_id: 0, quantity: 0, unit: "kemasan", unit_price: 0, notes: "" }]);
    } catch {
      toast({ variant: "destructive", title: "Error!", description: "Gagal memuat data faktur bahan." });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await nutritionIngredientInvoiceApi.update(Number(id), {
      ...
      toast({ variant: "success", title: "Berhasil!", description: "Faktur bahan gizi berhasil diperbarui." });
      navigate("/nutrition/invoices");
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Error!",
        description: error.response?.data?.error || "Gagal memperbarui faktur bahan gizi.",
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <PageShell>
      <PageHeader
        title="Edit Faktur Bahan Gizi"
        description="Perbarui data faktur pembelian bahan gizi beserta item bahan."
        icon={ReceiptText}
        actions={(
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button type="submit" form="nutrition-invoice-edit-form" size="sm" disabled={loading}>
      ...
      <PageContent className="flex-none pb-8">
        <form id="nutrition-invoice-edit-form" onSubmit={handleSubmit} className="space-y-6">
          <NutritionSectionPanel title="Informasi Faktur" description="Nomor faktur, tanggal, supplier, dan catatan umum faktur.">
          ...
          <NutritionSectionPanel title="Daftar Bahan Faktur" description="Pilih bahan dari master dan isi jumlah serta harga tiap item.">
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs text-muted-foreground">Tanpa pengurangan stok, untuk pencatatan faktur pembelian bahan.</p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input value={ingredientForm.name} onChange={(e) => setIngredientForm((prev) => ({ ...prev, name: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Kategori Bahan</Label>
                <Input value={ingredientForm.category} onChange={(e) => setIngredientForm((prev) => ({ ...prev, category: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Satuan Default</Label>
                <Combobox options={unitOptions} value={ingredientForm.default_unit} onValueChange={(value) => setIngredientForm((prev) => ({ ...prev, default_unit: value }))} placeholder="Pilih satuan..." />
              </div>
              <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 pt-6">
                <Switch checked={ingredientForm.is_active} onCheckedChange={(checked) => setIngredientForm((prev) => ({ ...prev, is_active: checked }))} />
                <Label>Bahan Aktif</Label>
              </div>
            </div>
            <div className="space-y-2">
              <Label>Catatan</Label>
              <Textarea value={ingredientForm.notes} onChange={(e) => setIngredientForm((prev) => ({ ...prev, notes: e.target.value }))} rows={2} />
            </div>
