...
import { PartografVisual } from "./partograf-visual";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border/60 bg-card text-card-foreground shadow-sm overflow-hidden">
        <div className="border-b border-border/50 bg-muted/40 px-4 py-3 flex items-center justify-between gap-2">
          <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            Data Partograf & Tindakan Persalinan
          </div>
        ...
        <div className="p-3 sm:p-4 space-y-6 bg-slate-50/40 dark:bg-transparent">
          {/* 1. INFORMASI UMUM & TAMBAHAN */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6">
            <div className="space-y-1">
              <Label className="text-xs">Ketuban pecah sejak</Label>
              <TimeInput
            ...
            <div className="space-y-1">
              <Label className="text-xs">Mules sejak jam</Label>
              <TimeInput
            ...
            <div className="space-y-1">
              <Label className="text-xs">BAB (x)</Label>
              <Input
                type="number"
                min="0"
                value={formData.catatan_kala_1?.bab || ""}
                onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, bab: e.target.value ? Number(e.target.value) : undefined })}
                disabled={isReadOnly}
                placeholder="Mis. 1"
              />
            </div>
            <div className="space-y-1">
              <Label className="text-xs">BAK (x)</Label>
              <Input
                type="number"
                min="0"
                value={formData.catatan_kala_1?.bak || ""}
                onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, bak: e.target.value ? Number(e.target.value) : undefined })}
                disabled={isReadOnly}
                placeholder="Mis. 2"
              />
            </div>
          ...
          <div className="pt-4 border-t border-border/40">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                <h4 className="text-[11px] font-semibold uppercase text-muted-foreground">Makan Terakhir</h4>
                <div className="grid grid-cols-4 gap-3">
                  <div className="col-span-1 space-y-1">
                    <Label className="text-xs">Pukul</Label>
                    <TimeInput
                      value={formData.catatan_kala_1?.makan_terakhir_jam || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, makan_terakhir_jam: e.target.value })}
                      disabled={isReadOnly}
                    />
                  </div>
                  <div className="col-span-2 space-y-1">
                    ...
                    <Input
                      value={formData.catatan_kala_1?.makan_terakhir_jenis || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, makan_terakhir_jenis: e.target.value })}
                      disabled={isReadOnly}
                      placeholder="Nasi & sayur"
                    />
                  </div>
                  <div className="col-span-1 space-y-1">
                    <Label className="text-xs">Porsi</Label>
                    <Input
                      value={formData.catatan_kala_1?.makan_terakhir_porsi || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, makan_terakhir_porsi: e.target.value })}
                      disabled={isReadOnly}
                      placeholder="1 porsi"
                    />
                  </div>
              ...
              <div className="space-y-3">
                <h4 className="text-[11px] font-semibold uppercase text-muted-foreground">Minum Terakhir</h4>
                <div className="grid grid-cols-4 gap-3">
                  <div className="col-span-1 space-y-1">
                    <Label className="text-xs">Pukul</Label>
                    <TimeInput
                      value={formData.catatan_kala_1?.minum_terakhir_jam || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, minum_terakhir_jam: e.target.value })}
                      disabled={isReadOnly}
                    />
                  </div>
                  <div className="col-span-2 space-y-1">
                    <Label className="text-xs">Jenis</Label>
                    <Input
                      value={formData.catatan_kala_1?.minum_terakhir_jenis || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, minum_terakhir_jenis: e.target.value })}
                      disabled={isReadOnly}
                  ...
                  <div className="col-span-1 space-y-1">
                    <Label className="text-xs">Porsi</Label>
                    <Input
                      value={formData.catatan_kala_1?.minum_terakhir_porsi || ""}
                      onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, minum_terakhir_porsi: e.target.value })}
                      disabled={isReadOnly}
                      placeholder="1 gelas"
                    />
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-border/40">
            <div className="space-y-2">
              <Label className="text-xs">Catatan Persalinan (Diplot ke Grafik Pembukaan)</Label>
              <Textarea
                value={formData.catatan_kala_1?.catatan_persalinan || ""}
                onChange={(e) => onChange("catatan_kala_1", { ...formData.catatan_kala_1, catatan_persalinan: e.target.value })}
                disabled={isReadOnly}
                rows={3}
                placeholder="Mis. Tanggal 22 Maret 2022 Pukul 17.40 WIB&#10;Bayi lahir spontan..."
                className="resize-none font-mono text-xs"
              />
            </div>
            <div className="space-y-2">
              <Label className="text-xs">Laporan Tindakan Persalinan</Label>
              <Textarea
          ...
          {/* 3. DATA PARTOGRAF TABLE */}
          <div className="pt-2 border-t border-border/40 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-[11px] font-semibold uppercase text-muted-foreground tracking-[0.1em]">
                Tabel Observasi per Jam/30 Menit
              </h4>
              {!isReadOnly && (
                <Button type="button" onClick={() => handleOpenModal()} variant="secondary" size="sm" className="h-7 text-xs gap-1.5 px-3">
            ...
            <div className="overflow-x-auto rounded-lg border border-border">
              <table className="w-full text-[11px] min-w-[700px]">
                ...
                <tbody className="divide-y divide-border">
                  {partografData.length === 0 ? (
                    <tr>
                      <td colSpan={17} className="p-6 text-center text-muted-foreground bg-card">Belum ada data partograf</td>
                    </tr>
                  ) : (
      ...
      <Dialog open={isVisualModalOpen} onOpenChange={setIsVisualModalOpen}>
        <DialogContent className="max-w-5xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>Diagram Partograf</DialogTitle>
          </DialogHeader>
          <div className="overflow-x-auto pb-4">
            <PartografVisual data={partografData} formData={formData} />
          </div>
        </DialogContent>
      </Dialog>
    </div>
