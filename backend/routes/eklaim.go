...
func RegisterEKlaimRoutes(r *gin.RouterGroup) {
	...
	eklaimLocal.Use(middleware.AuthMiddleware())
	{
		...
		eklaimLocal.POST("/:id/create-procedure-order", middleware.RequirePermission("eklaim.edit"), handlers.CreateCasemixProcedureOrder)
		...
		eklaimLocal.POST("/:id/sync-procedure-order-from-visit", middleware.RequirePermission("eklaim.edit"), handlers.SyncSingleProcedureOrderFromVisit)
		...
		eklaimLocal.POST("/:id/sync-claim-data", middleware.RequirePermission("eklaim.edit"), handlers.SyncClaimDataFromEKlaim)
