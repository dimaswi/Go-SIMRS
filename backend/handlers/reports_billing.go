...
func ReportDailyRevenue(c *gin.Context) {
	...
	db.Raw(`
		SELECT
			TO_CHAR(b.created_at, 'YYYY-MM-DD') AS tanggal,
	...
	db.Raw(`
		SELECT COALESCE(SUM(final_amount),0) AS total_tagihan,
			...
			COUNT(*) AS jumlah_billing
		FROM billings WHERE deleted_at IS NULL AND status != 'cancelled'
...
func ReportRevenueByPayment(c *gin.Context) {
	...
	db.Raw(`
		SELECT
			CASE
				WHEN b.payment_method = 'bpjs' THEN 'BPJS'
				WHEN b.payment_method = 'cash' THEN 'Umum/Cash'
				WHEN b.payment_method = 'insurance' THEN 'Asuransi'
				WHEN b.payment_method = 'debit' THEN 'Debit'
				WHEN b.payment_method = 'credit' THEN 'Kredit'
				WHEN b.payment_method = 'transfer' THEN 'Transfer'
				ELSE COALESCE(b.payment_method, 'Lainnya')
			END AS metode_bayar,
			COALESCE(SUM(b.final_amount), 0) AS total_tagihan,
			COALESCE(SUM(b.paid_amount), 0) AS total_bayar,
			COUNT(*) AS jumlah
...
func ReportRevenueByDoctor(c *gin.Context) {
	...
	db.Raw(`
		...
		WHERE b.deleted_at IS NULL AND b.status != 'cancelled'
		  AND b.created_at BETWEEN ? AND ? AND v.doctor_id IS NOT NULL
...
func ReportReceivables(c *gin.Context) {
	...
	if IsExcelExport(c) {
		...
		headers := []string{"No Billing", "Nama Pasien", "No RM", "Metode Bayar", "Total Tagihan", "Total Bayar", "Sisa Piutang", "Tgl Billing", "Status"}
		WriteExcelHeader(f, sheet, headers, styles.HeaderStyle)
		for i, row := range rows {
			r := i + 2
...
func ReportBillingByItemType(c *gin.Context) {
	...
	db.Raw(`
		...
		WHERE bi.deleted_at IS NULL AND b.deleted_at IS NULL AND b.status != 'cancelled'
		  AND b.created_at BETWEEN ? AND ?
